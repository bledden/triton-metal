...
| 0g | **macOS 26 (Tahoe) compatibility** | **DONE (validated on macOS 26.6, build 25G72).** `_infer_metal_version` runtime-probes each `-std=metalX.Y` for real loadability instead of guessing from SDK numbers, so it is immune to the 15.x→26.x SDK jump that broke PyTorch MPS version parsing. Confirmed on real 26.6 hardware: version probing, `compile_shader`, and `xcrun` invocations all work; full suite 1971 passed / 0 failed. | No code change (validation) | `triton_msl/backend/device_detect.py`, `triton_msl/backend/compiler.py` | 0b |
