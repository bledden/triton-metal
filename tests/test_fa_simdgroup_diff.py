...
from triton_msl.codegen._msl_templates import make_flash_attention_kernel_simdgroup, make_flash_attention_kernel_tiled
...
def test_simd_matches_scalar_and_torch(dt, causal, N):
    ...
    o_sd = torch.empty_like(q)
    o_sc = torch.empty_like(q)
    _run(
        make_flash_attention_kernel_simdgroup(HD, 32, 64, causal=causal, out_dtype=od),
        "flash_attention",
        q,
        k,
        v,
    ...
    _run(
        make_flash_attention_kernel_tiled(HD, 32, 32, Dc=64, causal=causal, out_dtype=od),
        "flash_attention",
        q,
        k,
        v,
