...
def _run_fast(dtype, M, N, K, seed=0):
    ...
    subprocess.check_call(
        ["xcrun", "-sdk", "macosx", "metal", "-c", p, "-o", d + "/k.air", "-std=metal3.2", "-O2"],
        stderr=subprocess.DEVNULL,
    )
    subprocess.check_call(
        ["xcrun", "-sdk", "macosx", "metallib", d + "/k.air", "-o", d + "/k.metallib"], stderr=subprocess.DEVNULL
    )
    dev = Metal.MTLCreateSystemDefaultDevice()
    lib, _ = dev.newLibraryWithURL_error_(Foundation.NSURL.fileURLWithPath_(d + "/k.metallib"), None)
    pso, _ = dev.newComputePipelineStateWithFunction_error_(lib.newFunctionWithName_("simdgroup_matmul_fast"), None)
    ...
    enc.dispatchThreadgroups_threadsPerThreadgroup_(Metal.MTLSizeMake(ng, 1, 1), Metal.MTLSizeMake(128, 1, 1))
    enc.endEncoding()
    cmd.commit()
