...
requires_all = pytest.mark.skipif(
    not (HAS_TORCH and HAS_MPS and HAS_TRITON and HAS_METAL), reason="Requires PyTorch MPS + Triton + Metal"
)
...
def test_mps_vector_add():
    ...
    actual = out.cpu()
    assert torch.allclose(actual, expected, atol=1e-5), f"Max diff: {(actual - expected).abs().max()}"
...
def test_mps_tensor_survives_triton():
    ...
    torch.mps.synchronize()
    assert torch.allclose(out.cpu(), torch.full((n,), 2.0)), f"Expected 2.0, got: {out[:8].tolist()}"
