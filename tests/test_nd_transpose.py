...
if HAS:
    ...
    def _trans4d(
        ...
        in_desc = tl.make_tensor_descriptor(
            base=In, shape=[s1, s2, s3, s4], strides=[s4 * s3 * s2, s4 * s3, s4, 1], block_shape=[s1, s2, s3, s4]
        )
        out_desc = tl.make_tensor_descriptor(
            base=Out, shape=[o1 * o2 * o3 * o4], strides=[1], block_shape=[o1 * o2 * o3 * o4]
        )
...
def test_nd_transpose(shape, perm, dt):
    ...
    assert torch.equal(Out, want), f"shape={shape} perm={perm} dt={dt}: mismatch"
