...
def make_ctx(top_level=None, all_ops=None, called_funcs=None, type_strs=None):
...
def test_case_names_are_unique():
    names = [c.name for c in rc.REFUSAL_CASES] + [c.name for c in rc.CONTEXTUAL_REFUSALS]
...
def test_clean_graph_is_not_refused():
    ...
    ops = [FakeOp("tt.load", id=1), FakeOp("arith.addf", id=2), FakeOp("tt.store", id=3)]
...
def test_noinline_dot_fires():
    ...
    ctx = make_ctx(top_level=[FakeOp("tt.call", id=1)], all_ops=[FakeOp("tt.call", id=1)], called_funcs=[callee])
    v = rc.check_all(ctx)
...
def test_nd_cat_join_fires_on_rank2():
    ...
    ctx = make_ctx(top_level=[cat], all_ops=[cat], type_strs={10: (32, 16)})  # rank-2 shape
    v = rc.check_all(ctx)
...
def test_nd_cat_join_allows_rank1():
    cat = FakeOp("tt.cat", id=5, operand_ids=[10])
    ctx = make_ctx(top_level=[cat], all_ops=[cat], type_strs={10: (64,)})  # rank-1 — supported
