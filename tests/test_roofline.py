...
def test_memory_bound_classification():
    ...
    n = 16 * 1024 * 1024
    bytes_moved = 3 * n * 4
    r = classify(bytes_moved, flops=n, seconds=bytes_moved / 1e9 / 137.5, dtype="fp32")
...
def test_fp16_roof_is_double_fp32():
    roofs = HardwareRoofs()
    assert math.isclose(roofs.compute_roof_tflops("fp16"), 2 * roofs.compute_roof_tflops("fp32"), rel_tol=1e-9)
...
def test_overridable_roofs():
    custom = HardwareRoofs(name="Custom", mem_bw_gbps=1000.0, fp32_tflops=50.0, fp32_is_estimate=False)
...
def test_normal_measurement_not_suspect():
    r = classify(3 * 16 * 1024 * 1024 * 4, 16 * 1024 * 1024, seconds=0.0014, dtype="fp32")
